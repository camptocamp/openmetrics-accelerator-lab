...
It simulates a very simple particle accelerator where a particle (represented by a moving ball) travels around a ring and receives periodic “kicks” to accelerate.
...
The project illustrates how operational metrics can be embedded directly into an application’s logic — allowing developers to observe its internal state and performance through Prometheus
...
* [http://localhost:8080](http://localhost:8080): code-server (VS Code in the browser)
...
4. Adjust the `KICK_POWER` parameter in `app.py` directly from code-server — Flask automatically reloads.
